...
export function generatePatientESP32Code(
  ...
  const ssid = customConfig?.ssid ?? 'NIRMAAN-2026';
  const password = customConfig?.password ?? 'Nirmaan25hr';
...
// Your laptop (server) IP on the Nirmaan 2026 network
const char* SERVER_HOST = "10.110.6.124";
const int   SERVER_PORT = 3000;
const char* DEVICE_ID   = "${patient.pillboxId || 'DSBOX-01'}";
...
void syncTimeFromNTP() {
  ...
  // ===================================================
  // Wi-Fi stays ON — needed for HTTP event reporting
  // ===================================================

  Serial.println();
  Serial.println("Wi-Fi staying connected for server reporting.");

  // Send initial ping to server so it knows device is online
...
// =====================================================
//           SEND LDR EVENT TO SERVER
// =====================================================

void sendEventToServer(int compartmentNum, int ldrValue) {

  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[EVENT] WiFi not connected — skipping server report.");
    return;
  }
  ...
  // Generate unique event ID using millis
  String eventId = String(DEVICE_ID) + "-C" + String(compartmentNum) + "-" + String(millis());

  String body = "{";
  body += "\"event_id\":\""   + eventId + "\",";
  body += "\"device_id\":\""  + String(DEVICE_ID) + "\",";
  body += "\"patient_id\":\"" + String(PATIENT_ID) + "\",";
  body += "\"compartment\":" + String(compartmentNum) + ",";
  body += "\"event_type\":\"COMPARTMENT_OPENED\",";
  body += "\"event_time\":\"" + String(millis()) + "\",";
  body += "\"ldr_value\":"   + String(ldrValue);
  body += "}";

  int httpCode = http.POST(body);
  Serial.print("[EVENT] Sent to server. Response code: ");
...
void checkDose1(
  ...
  if (dose1Active) {
    ...
    if (
      ...
      // Send real event to DoseSure server
      sendEventToServer(1, ldrValue);
